...
describe('interpreter', () => {
  it('enables autostart on spawned audiorepeaters', () => {
  ...
  it('sets window name based on command.hash', () => {
    const commands: Command[] = [
      {
        from: 'a',
        to: 'b',
    ...
    expect(
      args1.find(x => getWindowRegex(commands[0].hash).test(x))
    ).toBeTruthy()
    expect(
      args2.find(x => getWindowRegex(commands[1].hash).test(x))
    ).toBeTruthy()
  })

  it('sets input argument to command.from', () => {
    const commands: Command[] = [
      {
        from: 'a',
        to: 'b',
  ...
  it('sets output argument to command.to', () => {
    const commands: Command[] = [
      {
        from: 'a',
        to: 'b',
  ...
  it('adds command.properties as arguments', () => {
    const commands: Command[] = [
      {
        from: 'a',
        to: 'b',
